...
This holds for any regulator, biological or computational, by construction of the inequality, once the recovery assumption is granted. That assumption is exactly what "regulation" means: an outcome function that ignores the disturbance without the regulator having actually absorbed its variety is not regulating anything. With the assumption made explicit, the result is a triviality of the mathematics, not a discovery about minds, and it should be stated without further hedging.
...
The forecast-time variety is small. The disturbance variety is large. So the bound on forecast error is large. That much is Proposition 1 alone, and the proposition stops there: it floors the spread of the residual, and says nothing about which side of the estimate that residual falls on.

**The direction of the miss is a separate, stated fact, not a further consequence of the proposition** [Layer 2: Fit]. A regulator whose point estimate tracked the true conditional mean would not be systematically biased even at this same narrow forecast-time variety. Narrow variety floors the width of the error, not which way it points. The direction here follows from two further, ordinary facts about the case. First, unnoticed cost components are non-negative, so what a forecast misses can only add to the true cost, never subtract from it. Second, a prior fitted to a population of tasks is centered on the common case rather than the tail, so it tracks the *mode* of a distribution whose mass sits toward the expensive side, not its mean.

Given both, the residual lands where the unpriced tail sits: upward in cost, downward in the estimate.

Once both facts are in view, the observed factor-of-five underestimation stops looking surprising: it is roughly what you would expect when a regulator prices a heavy-shouldered cost distribution using a prior centered near its mode, not a pathology requiring a separate explanation.
...
A confidence spread five times wider than the pass-rate spread is what a narrow, idiosyncratic, forecast-time prior predicts, on that assumption. It is corroborating evidence for the same mechanism the token-cost miss already demonstrated, not an independent proof of it.
...
- Suppose a task's true cost is distributed over a range spanning roughly six binary orders of magnitude, from the cheapest plausible resolution to the most expensive, treated for this illustration as that many roughly equally likely magnitude classes. The disturbance variety in the cost dimension alone is then on the order of {% katex() %}V(\text{disturbance}) \approx 6{% end %} bits.
- Suppose the model's forecast-time prior, before it reads anything, can only sort a task into one of about four distinguishable cost buckets across that whole range, rather than placing it anywhere within a finer-grained scale. That is the regulator's own response variety, not the size of what is left over once a response is chosen: {% katex() %}V(\text{regulator}) \approx 2{% end %} bits.
...
This series is not the only place the underlying question, whether a human-machine correspondence reflects a real formal necessity or a coincidence of shape, is being asked with this level of formality. A 2026 result derives primacy and anchoring biases in both autoregressive language models and human working memory from a single mechanism, the causal-masking constraint sequential processing itself imposes. It comes with its own information bounds and its own validated fit across a dozen models {{ cite(ref="10", title="Wu, J., Guo, D. & Yiu, S.-M. (2026) -- Bias by Necessity: Impossibility Theorems for Sequential Processing with Convergent AI and Human Validation, arXiv:2605.08716") }}.

That is a different theorem, aimed at different biases, built on a mechanism, attention architecture, that this series does not use anywhere. It is not evidence for anything claimed here, and the two arguments do not corroborate each other's specific mechanism. What it does confirm is that the meta-question this section exists to discipline is a live one elsewhere too. It is being pursued by researchers with no stake in this series' own claims, using tools of their own.
...
That proof is not attempted here. Establishing a self-certification limit formally, for even one property, rigorously scoped as a Layer 1 bound before any claim about generalizing it further, is a separate undertaking from naming and pricing the gap.

Post 1 names the concept and states its cost. It does not yet show the cost is unavoidable. The definition given here is deliberately incomplete. Saying so is more useful than pretending this post closes a question it has not actually answered.
...
**F3 (the formalization itself).** A regulator is exhibited that drives outcome variety below {% katex() %}V(\text{disturbance}) - V(\text{regulator}){% end %}, in a discrete setting, or a continuous setting quantized at any fixed resolution with all three varieties computed at that same resolution. The proof is a statement about discrete entropy; raw continuous densities, where the recovery premise degenerates to differential entropy of negative infinity rather than zero, are outside what it covers, and are not a place to look for a counterexample.
...
This series accumulates one artifact across its four property posts, a ledger with one row added per post, assembled in full at the close of the fourth. The ledger is the series' running record, and it is also a deployment gate: a failed verdict on a property does not stop at "the agent lacks this property," it names the specific architectural response the failure demands. Here is the first row.
...
<sup>[3]</sup> Cover, T. M. & Thomas, J. A. (2006). *Elements of Information Theory,* 2nd edition. Wiley (subadditivity of entropy, Chapter 2).
...
<sup>[10]</sup> Wu, J., Guo, D. & Yiu, S.-M. (2026). *Bias by Necessity: Impossibility Theorems for Sequential Processing with Convergent AI and Human Validation.* arXiv:2605.08716.
