...
**A later note, added after a different series on this blog needed the same distinction.** Everything above is about many agents checking each other: independence between voters is what makes a headcount mean anything. [The Portable Mind](@/blog/2026-09-05/index.md) later asks the mirror question, whether a single agent can check itself, and finds the same requirement one level down: a verification signal only counts if it draws on something the thing it checks could not also have corrupted. Loeb's theorem gives that series one case where the answer is a proof rather than a measured correlation coefficient. Nothing here changes because of it. The two posts are checking the same requirement at two different scales, not restating each other. [Post 5](@/blog/2026-09-06/index.md) of that series later states the shared requirement precisely and shows this post's committee math and that series' single-agent result were computing the same criterion from opposite directions.
