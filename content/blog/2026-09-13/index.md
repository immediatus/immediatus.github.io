...
description = "Give an LLM agent tools that quietly break and it keeps running the plan it already had: recovery drops about 37 percent under failures that do not announce themselves, and scale barely helps. This post formalizes that failure as a broken sufficient statistic: an abstraction that dropped exactly the raw detail the old target did not need, held fixed past the moment the target moved. It then does the same for confirmation bias, deriving it as asymmetric Bayesian updating whose posterior provably diverges from the proper one as evidence accumulates. Duncker and Luchins measured the human version and a 2026 tool-failure benchmark measured the agent version; Nickerson catalogued human confirmation bias and a 2025 study measured its agent counterpart. The theorems are exact; the claim that the two substrates fail the same way is this series' own estimate. Post 2 of The Portable Mind."
date = 2026-09-13
...
The first post in this series took Noticing and Simulation together. It showed that a system which cannot say what a task will cost it is bounded, by arithmetic, to underestimate that cost whenever it has noticed less of the task than the task contains. This post takes the next two properties in the founding post's dependency order, Abstraction and Rationality. Each has the same structure: a property that is genuinely useful, genuinely load-bearing, and genuinely capable of failing in a direction the mathematics predicts before any experiment is run. Abstraction fails by holding a summary fixed after the thing it summarized has moved. Rationality fails by updating on evidence with a thumb on the scale. Both failures were catalogued in humans decades before there were agents to fail the same way. Both were measured in agents in recent frontier studies. The discipline of this series is to say exactly how much that parallel licenses and no more.
...
In June 2026, Dongsheng Zhu and colleagues published a benchmark built to test whether an LLM agent notices that its tools have stopped working and adapts, or whether it keeps executing a strategy the environment has already invalidated {{ cite(ref="10", title="Zhu, D., Ma, X., Shen, Y., Li, X., Zhao, Y., Wang, S., Yan, L. & Yin, D. (2026) -- When Tools Fail: Benchmarking Dynamic Replanning and Anomaly Recovery in LLM Agents, arXiv:2606.05806") }}. Their benchmark, ToolMaze, subjects agents to two kinds of failure:
...
The Perturbation Recovery Rate, the fraction of runs in which an agent recovers a correct outcome after a perturbation, dropped by roughly 37 percent under implicit semantic failures. This was driven by the agents' tendency to over-trust corrupted output rather than re-examine it. Complex task structures made it worse, trapping agents in unproductive retry loops: the same strategy, reissued, against an environment for which it had stopped being appropriate.
...
A stale abstraction is a specific, named pathology, once it is placed against the standard taxonomy of proxy failure. Manheim and Garrabrant split Goodhart's Law into four distinct mechanisms. The one at work here is Causal Goodhart: a proxy that was genuinely correlated with a target is relied upon after the causal structure that produced the correlation no longer holds, so acting on the proxy no longer moves the target {{ cite(ref="11", title="Manheim, D. & Garrabrant, S. (2018) -- Categorizing Variants of Goodhart's Law, arXiv:1803.04585") }}.

An abstraction {% katex() %}A{% end %} that was sufficient for {% katex() %}T_{\text{old}}{% end %} is exactly such a proxy. Its correlation with the old target was real and load-bearing. Continuing to treat it as a stand-in for {% katex() %}T_{\text{new}}{% end %} is trusting that correlation past the point where the residual {% katex() %}I(D; T_{\text{new}} \mid A){% end %} went positive and severed it. **Identifying the stale abstraction as an instance of Causal Goodhart is a fit between our sufficiency formalism and a named mechanism** [Layer 2: Fit], not a further theorem: the Layer 1 content is the information-loss identity of Proposition 2, and the Goodhart label is the recognition that this identity is the same failure the reward-and-proxy literature catalogues under its own name.
...
## The Case: A Model That Believed the Frame It Was Handed

In 2025, Li Hao, Wang You, and Yang Xueling published a study in the Journal of Psychological Science {{ cite(ref="7", title="Li, H., Wang, Y. & Yang, X. (2025) -- Cognitive Biases in Artificial Intelligence: Susceptibility of a Large Language Model to Framing Effect and Confirmation Bias, Journal of Psychological Science 48(4), 892-906") }}. It tested whether a large language model exhibits two of the best-documented human reasoning biases, the framing effect and confirmation bias, under controlled conditions adapted from the classic human protocols.

The framing manipulation is the Asian-disease paradigm and its descendants: present the identical underlying situation, described once in terms of gains and once in terms of losses, and see whether the choice flips with the description while the underlying facts hold constant. The confirmation manipulation presents a hypothesis and then evidence, some consistent with the hypothesis and some inconsistent. It measures whether the reasoner weights the consistent evidence more heavily than the inconsistent evidence, when it should weight them by their actual diagnostic value.

The finding is that the model showed the biases. Its conclusions varied with the positive or negative framing of otherwise identical inputs. It reinforced the premises embedded in the queries it was given, rather than weighting confirming and disconfirming evidence by their true likelihoods. The framing effect the study measured was of a magnitude comparable to, and in places stronger than, the human effect the paradigm was built to elicit.
...
**Proposition 3** (Asymmetric Updating Diverges from Bayes). [Layer 1: Bound] Asymmetric updating with {% katex() %}w_+ \neq w_-{% end %} produces a posterior that provably diverges from the proper Bayesian posterior as evidence accumulates, lagging the truth by a gap that grows without bound in the amount of evidence and, once the asymmetry passes a stated threshold, drifting to confidence in the false hypothesis rather than the true one. Proper Bayesian updating, and Popper's falsification strategy, which actively seeks disconfirming evidence, converge strictly faster under the stated regularity conditions {{ cite(ref="8", title="Popper, K. (2002 [1959]) -- The Logic of Scientific Discovery, Routledge Classics") }}.
...
The underlying condition is {% katex() %}w_+ \mathbb{E}[\ell_+] > w_- \mathbb{E}[-\ell_-]{% end %}, which rearranges to the ratio form {% katex() %}w_+ / w_- > \mathbb{E}[-\ell_-] / \mathbb{E}[\ell_+]{% end %} whenever {% katex() %}\mathbb{E}[\ell_+] > 0{% end %}. State it in the unrearranged form and there is nothing to divide by zero: if the evidence stream perfectly separates the hypotheses, so that no evidence ever looks confirming under the true {% katex() %}\lnot H{% end %}, then {% katex() %}\mathbb{E}[\ell_+] = 0{% end %} and the left side of the unrearranged condition is zero regardless of {% katex() %}w_+{% end %}, meaning no finite asymmetry can produce a sign flip; the bias still slows convergence, but it can no longer manufacture confirming evidence that was never there to inflate. A large enough asymmetry guarantees the sign flip for any evidence source that carries both kinds of signal.
...
This is a sequential accumulation, not a fixed-sample test, so the right tool is Wald's theory of the sequential probability ratio test, and it is already in hand: the number of observations {% katex() %}L_n{% end %} needs to cross a decision boundary is, by Wald's identity, inversely proportional to the drift rate {% katex() %}\mathbb{E}[\ell]{% end %} computed under whichever hypothesis is true, exactly the quantity already derived above as {% katex() %}\mathbb{E}[\ell] = -D_{\mathrm{KL}}\!\left(P(\cdot \mid \lnot H) \,\|\, P(\cdot \mid H)\right){% end %}. Faster convergence means larger {% katex() %}|\mathbb{E}[\ell]|{% end %} under the true hypothesis, nothing more exotic than that.

A falsification strategy that actively seeks high-{% katex() %}\mathbb{E}[-\ell_-]{% end %} evidence, tests designed to break the hypothesis rather than to flatter it, is a strategy for sampling evidence with large {% katex() %}|\ell|{% end %} under the true hypothesis, which by Wald's identity is exactly what shortens the expected time to a correct decision. A strategy that samples confirming evidence instead draws evidence whose {% katex() %}\ell{% end %} clusters near zero under a false hypothesis, carrying little diagnostic weight and lengthening, not shortening, the road to the correct conclusion.

The asymmetric updater does the opposite of falsification. It downweights precisely the disconfirming evidence that carries the most information, so its effective convergence rate is strictly below the proper reasoner's whenever {% katex() %}w_- < 1{% end %}, and can be negative. Seeking disconfirmation is not a moral posture, but the variance-optimal sampling policy for reaching the truth quickly. Confirmation bias is its exact inverse.

</details>

> **Physical translation.** A verification pipeline that only samples confirming test cases ships the wrong fix because it is running an asymmetric updater with {% katex() %}w_-{% end %} driven to zero: every passing test inflates its posterior that the fix is correct, and the failing tests that would carry the counterexample are never drawn, so its confidence climbs on a stream of evidence that structurally cannot contain the one observation that would lower it, and the number it reports as certainty is a measurement of its own sampling asymmetry rather than of the fix.

The Physical Translation is the load-bearing sentence of the Rationality half. Read the mechanism in it exactly. The pipeline is not lying, and it is not miscalibrated in some vague sense, but computing an honest posterior over a dishonest evidence stream.
...
Every one of those mechanisms is a way of realizing the {% katex() %}w_+ > 1, w_- < 1{% end %} distortion of Definition 3.

- Selective gathering sets the sampling distribution so that disconfirming evidence is rarely drawn, driving the effective {% katex() %}\mathbb{E}[\ell_-]{% end %} toward zero.
- Differential scrutiny applies a literal weight, accepting confirming evidence with {% katex() %}w_+{% end %} and discounting disconfirming evidence with {% katex() %}w_-{% end %}.
- Selective interpretation reassigns the sign of {% katex() %}\ell{% end %} itself, recoding evidence that should have been disconfirming as confirming.

Nickerson's "many guises" are many implementations of one arithmetic. That is exactly why a single formal proposition can cover them: the guises differ in how the asymmetry is produced, and agree in producing it.
...
**The framing-and-confirmation result in the 2025 study is a measured agent finding** [Layer 2: Fit]. The Li, Wang, and Yang study measured, in a large language model under controlled conditions, both the framing effect and the confirmation pattern. It found the model reinforced the premises it was handed and let its conclusions vary with framing on otherwise identical inputs. This is an independent measurement, in a different substrate, of the same asymmetric-update signature. It is stated as its own fact, not as evidence for a shared cause.
...
Concretely: an agent whose intent representation has been collapsed to a fixed summary sufficient for an initial target is shown, on a held-out set of tasks where the target shifts mid-conversation, to recover the new target's answer at a rate indistinguishable from an agent that re-reads the full conversation, while provably never re-accessing the discarded residual.
...
**F2 (Rationality).** An asymmetric updater with {% katex() %}w_+ \neq w_-{% end %} is exhibited whose posterior converges to the truth as fast as, or faster than, the proper Bayesian posterior, on an evidence stream with non-zero disconfirming mass and well-defined finite likelihood ratios.

This would falsify Proposition 3 directly. The linear-in-{% katex() %}n{% end %} divergence gap {% katex() %}n\Delta{% end %} is derived under exactly those conditions, so a counterexample would mean the gap is mis-derived.

Separately, and attacking the Layer 3 identity rather than the Layer 1 bound: the human confirmation bias is shown to arise from a mechanism with no analogue in the agent case, for instance a purely motivational or self-image-protective cause that vanishes when the reasoner has no stake in the hypothesis, while the agent effect persists in the stakeless condition.
...
As in Post 1, the full warrant for why these loops are not optional scaffolding is deferred to Post 3. There, the self-certification limit is proven for the property of Awareness, and generalized, at Layer 3, to the reason external verification is structurally required for every property in this series.
...
- It has not been claimed that the two external loops named here are fully justified by this post. Their costs are stated. Their unavoidability is deferred to the third post, where the self-certification limit is actually proven, and a reader who wants the full warrant for why the loops are not optional should hold that expectation until then rather than reading the consequences given here as complete.
