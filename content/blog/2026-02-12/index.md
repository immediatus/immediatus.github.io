...
> **Discretization requirement — arms must be qualitatively distinct, not a fine grid**: EXP3-IX is a discrete bandit. Arms should represent qualitatively distinct operating regimes, not a fine grid over a continuous range. For short edge-mission horizons (\\(T \leq 2000\\)), **\\(K \leq 8\\) per tuned parameter** keeps bandit regret from dominating. When precision requires \\(K > 16\\) arms, use gradient descent or Bayesian optimization instead.
...
- **Post-partition reconnect**: connectivity regime changes invalidate pre-partition UCB estimates. After reconnect, wait for HLC reconciliation ({% term(url="@/blog/2026-02-05/index.md#def-63", def="Drift-Quarantine Re-sync Protocol: procedure for re-integrating a node whose HLC has drifted beyond the Clock Trust Window after extended partition") %}Definition 63{% end %}, [Fleet Coherence Under Partition](@/blog/2026-02-05/index.md#def-63)) and Drift-Quarantine checks ({% term(url="@/blog/2026-02-05/index.md#def-64", def="Peer-Validation Layer: gossip mechanism where nodes cross-check state updates to detect and reject Byzantine insertions without central authority") %}Definition 64{% end %}) to complete before restoring EXP3-IX arm weights from the pre-partition checkpoint. Restoring weights before HLC validation is complete could bias the bandit toward actions calibrated to a stale fleet state.
...
**Bayesian confidence floor.** For any Bayesian-adjacent component (UCB, warm-start prior), define an explicit confidence floor \\(\varepsilon_\text{conf}\\): if the posterior uncertainty (inverse confidence) exceeds \\(1/\varepsilon_\text{conf}\\), the component is operating outside its validity envelope and must either escalate to EXP3-IX or contribute a low-confidence signal to \\(\Psi(t)\\). This closes the loop to [The Constraint Sequence and the Handover Boundary](@/blog/2026-02-19/index.md)'s handover trigger: the bandit cannot become a silent failure mode — it always declares its own uncertainty.
...
**Composed regret bound.** The delayed-feedback regret (this proposition) composes with the instantaneous EXP3-IX regret bound ({% term(url="#prop-60", def="EXP3-IX Regret Bound: EXP3-IX achieves minimax expected regret against any adaptive adversary, matching the deterministic lower bound") %}Proposition 60{% end %}) as follows: the composed bound is at most {% katex() %}\sqrt{2}{% end %} times the instantaneous regret bound, provided partition episodes are independent. When episodes are correlated — as during sustained adversarial conditions — the composed regret may exceed this bound by a factor proportional to the autocorrelation time \\(\tau_{\text{corr}}\\); practitioners should treat the {% katex() %}\sqrt{2}{% end %} factor as a lower bound on overhead in correlated regimes.
...
This is a sufficient gain-delay stability condition — bounding the product \\(K(a)\cdot\tau(a)\\) prevents phase crossover in the autonomic control loop — applied as a hard pre-filter on the action space before any Q-value comparison is made. {% term(url="@/blog/2026-01-29/index.md#prop-22", def="Closed-Loop Healing Stability: gain must stay below a delay-dependent ceiling; exceeding it causes oscillation rather than convergence") %}Proposition 22{% end %} (LTI bound; SMJLS analysis in [Self-Healing Without Connectivity](@/blog/2026-01-29/index.md) tightens this for time-varying gain) establishes the discrete-time analogue; the condition {% katex() %}K(a)\cdot\tau(a) < \pi/2{% end %} is the continuous-time sufficient criterion for phase margin.
