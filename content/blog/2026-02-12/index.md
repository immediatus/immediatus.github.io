...
> **UCB validity boundary.** UCB (Proposition 18) assumes a **stochastic stationary** environment: arm reward distributions are fixed over the mission horizon. This assumption breaks under:
...
> - **Post-partition reconnect**: connectivity regime changes invalidate pre-partition UCB estimates. After post-partition reconnect: wait for HLC reconciliation (Definition 42, [Fleet Coherence Under Partition](@/blog/2026-02-05/index.md#def-42)) and Drift-Quarantine checks (Definition 43) to complete before restoring EXP3-IX arm weights from the pre-partition checkpoint. Restoring weights before HLC validation is complete could bias the bandit toward actions calibrated to a stale fleet state.
>
> When the deployment decision rule (see section above) identifies an adversarial or non-stationary environment, **do not use UCB** — switch to EXP3-IX (Definition 33). If UCB is already deployed and the environment turns adversarial mid-mission, the warm-start prior collapse detector (Definition 96 blockquote) will signal the transition; initiate an EXP3-IX restart with uniform weights at that point.
>
...
> **Prior collapse under jamming.** The warm-start prior (Definition 96) injects \\(N_q\\) virtual observations from offline simulation into the initial weight distribution. Under nominal conditions this accelerates convergence by 40% (Proposition 70). Under sustained jamming or severe distributional shift, however, these priors become systematically wrong — the offline environment no longer represents the online reality — and the bandit is biased toward sub-optimal arms for approximately \\(N_q\\) live ticks before the adversarial EXP3-IX component accumulates enough evidence to override the prior.
>
...
> 3. **Declare** — Set the policy-confidence component of \\(\Psi(t)\\) to its minimum. This emits a low-\\(\Psi\\) meta-diagnostic to [The Constraint Sequence and the Handover Boundary](@/blog/2026-02-19/index.md#prop-52): the local autonomic loop admits it can no longer confidently navigate the contested environment. This is a *diagnostic signal*, not a handover command. Proposition 52 will evaluate this against \\(\Psi_\\text{fail}\\) to determine whether predictive handover preparation should begin. Step 4 continues EXP3-IX re-learning from uniform weights; handover is not immediate — it is conditional on Proposition 52's criterion being met.
> 4. **Re-learn** — EXP3-IX continues with uniform weights from the reset point. Proposition 34's adversarial regret bound applies from reset onward; no ergodicity or stationarity assumption is required. The warm-start prior was a performance optimisation, not a safety dependency.
>
