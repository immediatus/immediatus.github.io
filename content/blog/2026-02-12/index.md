...
*(Notation: throughout this section \\(\\alpha > 0\\) denotes the exponential convergence rate with units {% katex() %}\text{s}^{-1}{% end %}. This is distinct from the false-alarm rates {% katex() %}\alpha_Q, \alpha_{\text{corr}}{% end %} used in the Q-change detector ({% term(url="#prop-62", def="Q-Change Detection Delay Bound: Adversarial Non-Stationarity Detector identifies distribution shifts within a bounded number of rounds inversely proportional to the shift magnitude") %}Proposition 62{% end %}), the Beta prior shape \\(\\alpha_k\\) used in {% term(url="#term-thompson", def="Bayesian bandit algorithm maintaining Beta posteriors over arm reward probabilities; samples to select arms, encoding uncertainty naturally and composing with gossip-shared priors after reconnection") %}Thompson Sampling{% end %} ({% term(url="#prop-60", def="EXP3-IX Regret Bound: EXP3-IX achieves minimax expected regret against any adaptive adversary, matching the deterministic lower bound") %}Proposition 60{% end %}), and the significance level \\(\\alpha\\) used in hypothesis tests. Context and subscripts differentiate all four uses within this article. Across the series, additional subscripted \\(\\alpha\\) forms appear in [Self-Healing Without Connectivity](@/blog/2026-01-29/index.md) ({% katex() %}\alpha_{\text{margin}}{% end %}, {% katex() %}\alpha_{\text{EMA}}{% end %} for gain scheduling; {% katex() %}\alpha_{\text{heal}}{% end %} for resource budgeting; {% katex() %}\alpha_{\text{age}}{% end %} for priority decay) — all distinct from the uses above.)*
...
> **UCB validity boundary.** UCB ({% term(url="#prop-58", def="UCB Regret Bound: Upper Confidence Bound achieves sub-linear cumulative regret against a stationary adversary, growing as the square root of rounds times arms times a log factor") %}Proposition 58{% end %}) assumes a **stochastic stationary** environment: arm reward distributions are fixed over the mission horizon. This assumption breaks under:
...
**Definition 80** (Adversarial Markov Game). An adversarial connectivity game is a 6-tuple {% katex() %}\mathcal{G} = (S, A, B, Q, R, \gamma_V){% end %} where:
...
> **Prior collapse under jamming.** The warm-start prior ({% term(url="#def-82", def="Capability-Hierarchy Warm-Start Prior: bandit initial weights from offline simulation, reducing rounds needed to reach convergence by roughly forty percent") %}Definition 82{% end %}) injects \\(N_q\\) virtual observations from offline simulation into the initial weight distribution. Under nominal conditions this accelerates convergence by 40% ({% term(url="#prop-61", def="Warm-Start+Contextual Regret Bound: reduces rounds-to-convergence by 40% vs. cold start (1312 vs 2233 rounds for RAVEN) via offline warm-start prior") %}Proposition 61{% end %}). Under sustained jamming or severe distributional shift, however, these priors become systematically wrong — the offline environment no longer represents the online reality — and the bandit is biased toward sub-optimal arms for approximately \\(N_q\\) live ticks before the adversarial EXP3-IX component accumulates enough evidence to override the prior.
>
> **Detecting prior collapse: Strategic Amnesia trigger.** Monitor the KL divergence between the empirical arm-reward distribution observed in the field and the distribution implied by the warm-start prior:
>
> {% katex(block=true) %}D_{\text{KL}}(\hat{P}_{\text{obs}}(t) | P_{\text{prior}}) = \sum_{i=1}^{K} \hat{p}_i(t) \ln \frac{\hat{p}_i(t)}{p_i^{\text{prior}}}{% end %}
>
> where {% katex() %}\\hat{p}_i(t){% end %} is the empirical reward frequency of arm \\(i\\) over a sliding window of \\(W = 20\\) ticks, and {% katex() %}p_i^{\text{prior}} \propto w_i(0; q){% end %} is the normalised warm-start weight. Under nominal conditions \\(D_{\text{KL}} \approx 0\\). Under sustained jamming, the adversary shifts rewards away from the arms the offline simulation favoured, driving \\(D_{\text{KL}} > 0\\). {% katex() %}\hat{P}_\text{obs}{% end %} is computed from the **true rewards** \\(r_s\\) (the unmasked outcome after the adversary's strategy shift \\(\xi_s\\) is applied), not from the exploration-randomized arm selections. This ensures \\(D_\text{KL}\\) detects changes in the adversary's effective strategy, not fluctuations caused by EXP3-IX's own exploration distribution.
>
> **Strategic Amnesia — four-step circuit breaker.** When {% katex() %}D_{\text{KL}}(\hat{P}_{\text{obs}} | P_{\text{prior}}) > \varepsilon_{\text{conf}}{% end %} persists for \\(n_{\\text{persist}} \geq 5\\) consecutive ticks (\\(\varepsilon_{\\text{conf}} = 0.30\\) nats for RAVEN):
>
> 1. **Detect** — D_KL breach confirmed: the offline simulation is no longer a valid model of the contested environment.
> 2. **Amnesia** — Reset \\(w_i \to 1/K\\) (uniform); discard all \\(N_q\\) virtual observations. The "ghost of the lab" is erased (i.e., the offline simulation prior is discarded; no past virtual observations inform the reset weight vector).
> 3. **Declare** — Set the policy-confidence component of \\(\Psi(t)\\) to its minimum. This emits a low-\\(\Psi\\) meta-diagnostic to [The Constraint Sequence and the Handover Boundary](@/blog/2026-02-19/index.md#prop-74): the local autonomic loop admits it can no longer confidently navigate the contested environment. This is a *diagnostic signal*, not a handover command. {% term(url="@/blog/2026-02-19/index.md#prop-74", def="Predictive Handover Criterion: edge node should initiate autonomous-mode handover when the partition accumulator exceeds the 95th-percentile partition duration") %}Proposition 74{% end %} will evaluate this against \\(\Psi_\\text{fail}\\) to determine whether predictive handover preparation should begin. Step 4 continues EXP3-IX re-learning from uniform weights; handover is not immediate — it is conditional on {% term(url="@/blog/2026-02-19/index.md#prop-74", def="Predictive Handover Criterion: edge node should initiate autonomous-mode handover when the partition accumulator exceeds the 95th-percentile partition duration") %}Proposition 74{% end %}'s criterion being met.
> 4. **Re-learn** — EXP3-IX continues with uniform weights from the reset point. {% term(url="#prop-60", def="EXP3-IX Regret Bound: EXP3-IX achieves minimax expected regret against any adaptive adversary, matching the deterministic lower bound") %}Proposition 60{% end %}'s adversarial regret bound applies from reset onward; no ergodicity or stationarity assumption is required. The warm-start prior was a performance optimisation, not a safety dependency.
>
> **Re-entry hysteresis.** Once Strategic Amnesia fires and weights reset to uniform, the circuit breaker is considered **cleared** only when {% katex() %}D_{\text{KL}}(\hat{P}_\text{obs} | P_\text{prior}) < \varepsilon_\text{conf} - \varepsilon_\text{hyst} = 0.25{% end %} nats and remains below this level for \\(N_\text{persist} = 5\\) consecutive ticks. This prevents rapid oscillation when D_KL hovers near the trigger threshold — a rapidly oscillating adversary could otherwise cause repeated Amnesia/Re-learn cycles that degrade convergence.
>
> **Key insight.** EXP3-IX does not need Bayesian convergence to remain safe — it is adversarially robust by construction. Strategic Amnesia transforms the warm-start prior from a potential liability into a manageable bias: under nominal conditions it accelerates convergence; under jamming it is cleanly discarded.
>
> **Notation warning.** \\(\varepsilon_\text{conf} = 0.30\\) nats and \\(\Psi_\text{fail} = 0.30\\) (defined in [The Constraint Sequence and the Handover Boundary](@/blog/2026-02-19/index.md#prop-74)) share the same numeric value but are **independent parameters with different units and different operational responses**. \\(\varepsilon_\text{conf}\\) is measured in nats (information-theoretic; maximum for K = 5 arms is \\(\ln 5 \approx 1.61\\) nats; 0.30 nats ≈ 19% of maximum). \\(\Psi_\text{fail}\\) is dimensionless (confidence score; range [0, 1]). The numeric coincidence is accidental — field calibration may change either value independently without affecting the other.

<span id="def-83"></span>
**Definition 83** (Connectivity-Pruned Contextual Arm Set). *Let {% katex() %}b_C(t) = \min(3,\; \lfloor 4 C(t) \rfloor) \in \{0,1,2,3\}{% end %} be the 2-bit connectivity bucket (same quantization scheme as Definition 87). Define the minimum connectivity requirement for arm \\(i\\) as the bucket threshold {% katex() %}\bar{b}_i{% end %} below which arm \\(i\\) is physically infeasible. The active arm set is:*

{% katex(block=true) %}
\mathcal{A}(t) = \bigl\{\,i \in \{1,\ldots,K\} : b_C(t) \geq \bar{b}_i\,\bigr\}
{% end %}

*The EXP3-IX selection and weight-update rules of {% term(url="#def-81", def="EXP3-IX Algorithm: adversarial bandit with implicit exploration achieving minimax regret against any adaptive adversary") %}Definition 81{% end %} are restricted to \\(\mathcal{A}(t)\\): inactive arms are neither pulled nor updated; their weights freeze at the last active value and resume when \\(C(t)\\) recovers.*
...
**Definition 85** (Delayed-Feedback EXP3-IX). Let the feedback delay for pull at partition onset \\(s\\) be {% katex() %}d_s = T_N^{(s)}{% end %} — the actual partition duration, observed only at partition end. Define batch window {% katex() %}B = \lceil E[T_N] \rceil{% end %} (one expected partition, LUT-approximated per Definition 13). \\(T_N\\) is the partition duration random variable from Definition 13; \\(k_N\\) is the same shape parameter as Definition 14. The **pending buffer** \\(\mathcal{P}\\) holds \\((k_s,\\; s)\\) pairs for pulls awaiting reward; capacity is bounded by {% katex() %}|\mathcal{P}| \leq \lfloor Q_{0.95} / T_{\min} \rfloor{% end %} entries (static upper bound; MCU-allocatable). At each partition event \\(s\\):

1. **Pull**: select arm {% katex() %}k_s \in \{0.3, 0.4, \ldots, 1.0\}{% end %} per {% term(url="#term-exp3", def="Exponential Weights algorithm for adversarial bandits with implicit exploration; achieves minimax regret against adaptive adversaries") %}EXP3-IX{% end %} weights; append \\((k_s,\\; s)\\) to \\(\mathcal{P}\\).
2. **Receive** (at partition end, time \\(s + d_s\\)): compute true reward {% katex() %}r_s = -\max(0,\; T_{\text{acc}}^{(s)} - E[T_N \mid k_s]) / Q_{0.95}(k_s){% end %} ({% term(url="@/blog/2026-01-15/index.md#def-14", def="Adaptive Weibull Shape Parameter: bandit algorithm that tunes the Weibull shape parameter online from observed partition durations") %}Definition 14{% end %} reward signal); remove \\((k_s,\\; s)\\) from \\(\mathcal{P}\\).
...
**Proposition 64** (Survival Invariant). Under Safe-\\(\varepsilon\\)-Greedy (Definition 90), the selected action satisfies {% katex() %}K(a_t)\cdot\tau(a_t) < \pi/2{% end %} whenever {% katex() %}\mathcal{U}_{\text{safe}}(t) \neq \emptyset{% end %}. When {% katex() %}\mathcal{U}_{\text{safe}}(t) = \emptyset{% end %}, the deterministic \\(L_0\\) policy is executed, which satisfies the survival constraint by [Proposition 8 (Hardened Hierarchy Fail-Down)](@/blog/2026-01-15/index.md#prop-8).

*Proof.* Step 1 constructs {% katex() %}\mathcal{U}_{\text{safe}}{% end %} by excluding all actions with {% katex() %}K(a)\cdot\tau(a) \geq \pi/2{% end %}. Both branches of Step 3 — exploration (uniform over {% katex() %}\mathcal{U}_{\text{safe}}{% end %}) and exploitation ({% katex() %}\arg\max{% end %} over {% katex() %}\mathcal{U}_{\text{safe}}{% end %}) — select exclusively from this set. The goto-Step-10 branch in Step 2 executes {% katex() %}\pi_{L_0}{% end %}, whose gains are pre-validated during Phase-0 attestation to satisfy \\(K\cdot\tau(a) < \pi/2\\); [{% term(url="@/blog/2026-01-15/index.md#prop-8", def="Hardened Hierarchy Fail-Down: when an authority tier fails, control automatically delegates to the next lower tier without service interruption") %}Proposition 8{% end %} (Hardened Hierarchy Fail-Down)](@/blog/2026-01-15/index.md#prop-8) ensures the \\(L_0\\) tier remains reachable when all autonomic actions are infeasible. \\(\square\\)
...
**Counter-scenario**: Systems that fail catastrophically under stress are not measured post-stress. Only survivors contribute to \\(\mathbb{A}\\) estimate, inflating apparent {% term(url="#def-79", def="System property where performance improves after stress exposure rather than merely recovering; each failure event yields better-calibrated parameters — the system at day 30 outperforms the system at day 1") %}anti-fragility{% end %}. Detection: compare {% katex() %}n_{\text{stressed}}{% end %} to {% katex() %}n_{\text{measured}}{% end %}. Mitigation: account for failure cases as \\(\mathbb{A} = -\infty\\) or excluded with explicit note.
