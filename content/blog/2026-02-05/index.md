...
<span id="def-57"></span>
**Definition 57** (State Divergence). *For system states \\(\Sigma_A\\) and \\(\Sigma_B\\) (state \\(\Sigma_A\\) denotes the key-value pair set of node A) represented as key-value pairs, the divergence \\(D(\Sigma_A, \Sigma_B)\\) is the normalized symmetric difference:*
...
**Proposition 41** (Divergence Growth Rate). *If state-changing events arrive according to a Poisson process with rate \\(\lambda\\), the expected divergence after partition duration \\(\tau\\) is:*
...
- **Parameters**: {% katex() %}\lambda{% end %} = update events per second (0.01–5 Hz for edge fleets); {% katex() %}\tau{% end %} = expected partition duration in hours.
...
- **Parameters**: {% katex() %}|S|{% end %} = object count; {% katex() %}D_{\text{worst}}{% end %} from burst-corrected {% term(url="#prop-41", def="Divergence Growth Rate: state divergence grows at a fixed rate during partition; expected divergence at reconnection scales with the update rate and expected partition duration") %}Proposition 41{% end %}; {% katex() %}b_{\text{item}} = 64\text{--}256{% end %} bytes per object.
...
**Definition 59** (Clock Trust Window). *Given hardware clock drift rate {% katex() %}\delta_{\max}{% end %} (fractional drift rate, dimensionless; e.g., 1 ppm = 10⁻⁶ — a constant from the hardware manufacturer's datasheet, not a tunable system parameter) and HLC skew bound {% katex() %}\varepsilon{% end %} from Proposition 45, the Clock Trust Window is:*
...
- **Parameters**: {% katex() %}\delta_{\max}{% end %} from hardware datasheet (GPS: 1 ppm; TCXO: 2–24 ppm; crystal: 50–200 ppm; RC oscillator: 1%–12%); {% katex() %}\varepsilon{% end %} from {% term(url="#prop-45", def="HLC Causal Ordering Properties: HLC maintains the happens-before relation and wall-clock proximity, bounding clock skew to a sum of the drift and network delay bounds") %}Proposition 45{% end %} calibration.
...
**Proposition 43** (LWW-to-Causal Pivot Correctness). *The pivot from HLC ordering {% katex() %}\prec{% end %} to logical ordering {% katex() %}\prec_{\mathrm{logic}}{% end %} at {% katex() %}T_{\mathrm{acc}} > T_{\mathrm{trust}}{% end %} (Definition 59) satisfies:*
...
*Proof.* Property 1: {% katex() %}\prec_{\mathrm{logic}}{% end %} uses {% katex() %}(c_j, n_j){% end %} — sub-components of {% katex() %}\prec_{\mathrm{ext}}{% end %} ({% term(url="#def-62", def="HLC-Aware CRDT Merge Function: CRDT merge using HLC timestamps for causal ordering, resolving conflicts correctly even when physical clocks have drifted") %}Definition 62{% end %}, defined below in the Hybrid Logical Clocks section of this post). Any pair with {% katex() %}h_1 \prec_{\mathrm{logic}} h_2{% end %} also satisfies {% katex() %}h_1 \prec_{\mathrm{ext}} h_2{% end %} — logical dominance is a necessary condition for causal dominance. Incorrectly concurrent pairs resolve via CRDT join, which is monotone by {% term(url="#def-58", def="CRDT (Conflict-free Replicated Data Type): data structure where all concurrent updates merge deterministically without coordination, enabling convergent consistency under partition") %}Definition 58{% end %} (semilattice). Property 2: {% katex() %}\prec_{\mathrm{logic}}{% end %} is a total preorder on {% katex() %}(c, n){% end %} pairs; every pair is comparable or equal-then-tied. Property 3: follows from Property 1. Property 4: after {% term(url="#def-63", def="Drift-Quarantine Re-sync Protocol: procedure for re-integrating a node whose HLC has drifted beyond the Clock Trust Window after extended partition") %}Definition 63{% end %} Phase 2 (HLC repair), {% katex() %}l_j{% end %} is corrected; Phase 3 (causality audit) reclassifies previously-concurrent pairs; {% term(url="#def-58", def="CRDT (Conflict-free Replicated Data Type): data structure where all concurrent updates merge deterministically without coordination, enabling convergent consistency under partition") %}Definition 58{% end %} idempotency guarantees re-merge produces the same join result. \\(\square\\)

- **Computes**: Formal correctness guarantees for the physical-to-logical clock pivot under long partitions.
- **Apply when**: Verify at integration time that all CRDT types satisfy {% term(url="#def-58", def="CRDT (Conflict-free Replicated Data Type): data structure where all concurrent updates merge deterministically without coordination, enabling convergent consistency under partition") %}Definition 58{% end %} semilattice properties before deploying the {% term(url="#def-59", def="Clock Trust Window: maximum elapsed partition time before the HLC falls back to pure logical ordering to prevent causal inversions") %}Definition 59{% end %} pivot.
- **Parameters**: {% katex() %}\varepsilon{% end %}, {% katex() %}T_{\mathrm{trust}}{% end %} from {% term(url="#def-59", def="Clock Trust Window: maximum elapsed partition time before the HLC falls back to pure logical ordering to prevent causal inversions") %}Definition 59{% end %}; all CRDT types must satisfy {% term(url="#def-58", def="CRDT (Conflict-free Replicated Data Type): data structure where all concurrent updates merge deterministically without coordination, enabling convergent consistency under partition") %}Definition 58{% end %} (idempotency, commutativity, associativity).
- **Prevents**: Pivot-induced data loss — Properties 1–12 prove the pivot only adds CRDT merges, never removes causally ordered updates.
...
> **Notation**: \\(\tau_{\max}\\) here denotes maximum one-way network message delivery time (milliseconds to seconds). This is distinct from the staleness time constant \\(\tau_{\text{stale}}^{\max}\\) in [Self-Healing Without Connectivity](@/blog/2026-01-29/index.md#def-45), which measures the observation expiry window (minutes to hours).
...
**Proposition 46** (Re-sync Correctness). *The Drift-Quarantine Protocol (Def 42) guarantees:*
...
<span id="def-67"></span>
**Definition 67** (Semantic Commit Order). *For two concurrent update records {% katex() %}u_1 = (v_1, V_1, p_1, \mathrm{id}_1){% end %} and {% katex() %}u_2 = (v_2, V_2, p_2, \mathrm{id}_2){% end %} — where \\(V_i\\) is the vector clock (Definition 60), \\(p_i \in \mathbb{Z}^+\\) is the application-assigned semantic priority, and {% katex() %}\mathrm{id}_i = (\mathrm{tier}_i, \mathrm{nid}_i){% end %} is the authority-tier identity (Definition 68, with {% katex() %}\mathrm{nid}{% end %} fixed at manufacturing) — the Semantic Commit Order \\(\prec\\) is determined by applying the following rule in sequence until a winner is found:*
...
**Proposition 50** (Tombstone Pruning Safety Bound). *In an \\(n\\)-node fleet with {% term(url="@/blog/2026-01-22/index.md#def-24", def="Epidemic dissemination protocol where each node contacts random neighbors to propagate state; convergence guaranteed in logarithmic rounds by Proposition 12") %}gossip{% end %} rate \\(\lambda\\), mesh diameter \\(D\\), and per-message loss probability {% katex() %}p_{\mathrm{loss}}{% end %}, the expected time until a tombstone satisfies the global acknowledgement condition is bounded by the convergence time from Proposition 13. At steady state, the unpruned tombstone count and memory footprint satisfy:*
...
- **Apply when**: Replace {% term(url="#def-70", def="Delta-Sync Protocol: gossip protocol transmitting only state deltas rather than full state, reducing sync bandwidth proportionally per round") %}Definition 70{% end %}'s {% katex() %}\Psi_i{% end %} with {% katex() %}\tilde{\Psi}_i{% end %} at every reconnection handshake; no per-delta-item changes required.
- **Parameters**: {% katex() %}l_i{% end %}, {% katex() %}c_i{% end %} from {% term(url="#def-61", def="Hybrid Logical Clock (HLC): clock combining physical and logical timestamps, advancing on message receipt to maintain causal ordering without NTP") %}Definition 61{% end %} send rule; {% katex() %}T_{\mathrm{acc},i}{% end %} from {% term(url="@/blog/2026-01-15/index.md#def-15", def="Partition Duration Accumulator: contiguous time spent in the disconnected regime; resets on partition end; input to threshold adaptation and the Weibull Circuit Breaker") %}Definition 15{% end %} accumulator; all three fields updated at each MAPE-K tick.
...
| :--- | :--- | :--- | :--- | :--- |
| 0–12 | {% katex() %}l_i{% end %} — HLC watermark | 4 B | Definition 61 send rule | Apply Def 40 receive rule; check Prop 41 anomaly condition |
| 4–24 | {% katex() %}c_i{% end %} — HLC counter | 2 B | Definition 61 send rule | Merge into local HLC counter |
| 6–22 | {% katex() %}T_{\mathrm{acc},i}{% end %} — partition age | 4 B | Definition 15 accumulator | Select {% katex() %}\prec{% end %} vs {% katex() %}\prec_{\mathrm{logic}}{% end %} (Def 98); decide quarantine (Def 42) |
...
| 12–79 | {% katex() %}h_{\mathrm{sfx},i}{% end %} — hash suffix | 4 B | Definition 29 chain suffix | Compare to local suffix; divergence within one tick = metric corruption |
| 16 | {% katex() %}\mathtt{flags}_i{% end %} | 1 B | Computed each tick | See flag table below |
| 17–94 | padding | 3 B | — | 4-byte boundary alignment |
...
| 2:1 | `zt_state` | 00 = OBSERVE, 01 = WAKEUP, 10 = ACTIVE ([Definition 96](@/blog/2026-02-19/index.md#def-96) Zero-Tax state) |
...
- **Apply when**: Replace {% term(url="#def-71", def="Causality Header: 8-byte gossip field encoding HLC timestamp and trust-validity bit, enabling causal ordering without NTP in every protocol message") %}Definition 71{% end %}'s {% katex() %}\tilde{\Psi}_i{% end %} with the UAH at every gossip exchange; all fields updated at each MAPE-K tick.
- **Parameters**: {% katex() %}\rho_{q,i}{% end %} encoded as unsigned byte (0 = margin 0.0, 255 = margin 1.0); {% katex() %}h_{\mathrm{sfx},i}{% end %} = last 4 bytes of {% term(url="@/blog/2026-01-22/index.md#def-29", def="Adaptive Gossip Rate Controller: feedback controller adjusting gossip fanout based on measured convergence lag, preventing under- and over-gossiping") %}Definition 29{% end %} chain; {% katex() %}q_i \in \{0,1,2,3,4\}{% end %} matching L0–L4.
...
If {% katex() %}\mathrm{RTT}_{\text{probe}} > T_{\text{timeout}}{% end %} (default: 2 s) or {% katex() %}\hat{B}_{n} < B_{\min}{% end %} (default: 9.6 kbps): set {% katex() %}\hat{B}_{n} = B_{\min}{% end %} and flag as **marginal link** — only Service Level \\(\mathcal{L}_1\\) items ({% term(url="#def-75", def="QoS Byte Budget: tier-reservation scheme (50% safety tier, cascading remainder) preventing starvation of critical updates under bandwidth pressure") %}Definition 75{% end %}) are transmitted. Probe cost: {% katex() %}L_{\text{probe}} + L_{\text{ACK}} \leq 560{% end %} bytes, 1 RTT — negligible against any usable link.

<span id="def-75"></span>

**Definition 75** (QoS Byte Budget). Note: 'service level' here refers to capability-level tiers (\\(\mathcal{L}_1\\)–\\(\mathcal{L}_4\\) from [Self-Healing Without Connectivity](@/blog/2026-01-29/index.md)), not the authority-tier concept (\\(Q_j\\) from Definition 68). Given probed bandwidth \\(\hat{B}\\) and connectivity window \\(T_W\\), the **total byte budget** (fingerprint overhead pre-deducted per Definition 70 Phase 1) is:
...
> **Note**: 'service level' here refers to the capability level (\\(\mathcal{L}_0\\)–\\(\mathcal{L}_4\\)) that reserves this bandwidth allocation — distinct from the decision-scope *authority tier* (\\(\mathcal{Q}_j\\)) of {% term(url="#def-68", def="Authority Tier: decision-scope hierarchy from node to cluster to fleet to command; higher tiers require higher connectivity, and partitions trigger delegation to lower tiers") %}Definition 68{% end %}.
...
**Proposition 54** (Sync Stability Bound). During Definition 70 Phase 3, monitor ACK round-trip time {% katex() %}\mathrm{RTT}(t){% end %} continuously. A **saturation event** is declared when:
...
- **Parameters**: {% katex() %}T_0 = 100{% end %} ms base backoff interval; {% katex() %}N_{\text{sat}}{% end %} = saturation event count per session (0–12 typical); {% katex() %}T_{\text{sync}}{% end %} = baseline sync time.
...
*Proof sketch.* Each backoff period \\(T_0 \cdot 2^n\\) is bounded. The geometric sum {% katex() %}\sum_{n=0}^{N_{\text{sat}}-1} T_0 \cdot 2^n = T_0 \cdot (2^{N_{\text{sat}}} - 1){% end %} gives total backoff overhead. The Service Level \\(\mathcal{L}_1\\) safety invariant follows from the ordering guarantee of {% term(url="#def-75", def="QoS Byte Budget: tier-reservation scheme (50% safety tier, cascading remainder) preventing starvation of critical updates under bandwidth pressure") %}Definition 75{% end %} (Service Level \\(\mathcal{L}_1\\) transmitted first) and {% katex() %}T_1 = 0.5\,\Omega / \hat{B} \leq 0.5 \cdot T_W{% end %}, so Service Level \\(\mathcal{L}_1\\) finishes well before the first backoff fires at {% katex() %}T_0 = 100\,\text{ms}{% end %}. \\(\square\\)
...
- **Cluster Alpha** (Vehicles 1–26): assessed Sector 7 as the primary threat. Dispatched Vehicle 4 north. Vehicle 4 expended its full kit — 8 controlled detonations. Kit cannot be restocked until the base resupply depot is reached.
- **Cluster Bravo** (Vehicles 7–58): independently reached the same threat assessment. Attempted to dispatch Vehicle 4 — found it absent from local mesh. Concluded it had self-tasked and proceeded to Sector 7 via alternate route, deploying their own improvised countermeasures at \\(3\times\\) the time cost.
...
4. If \\(u \leq P(a_i \mid \tau_k)\\): emits an **intent token** {% katex() %}\langle \tau_k,\, i,\, t \rangle{% end %} via the {% term(url="@/blog/2026-01-22/index.md#def-24", def="Peer-to-peer protocol where each node periodically exchanges state with random neighbors; health information spreads fleet-wide with mathematically bounded delay and no central coordinator") %}gossip{% end %} protocol.
...
- **Parameters**: {% katex() %}n{% end %} = fleet size; {% katex() %}P(a) = p^*{% end %} from {% term(url="#def-76", def="Redundant-Omission Loss Function: cost function penalizing both missed detections and redundant transmissions, used to optimize gossip fanout under bandwidth constraints") %}Definition 76{% end %}; example: {% katex() %}n=12{% end %}, {% katex() %}P(a)=0.5 \to P(\text{omission}) \approx 0.02\%{% end %}.
...
4. **L2 delegation validated**: Vehicles 6–58 operated effectively for 45 minutes under local lead authority.
...
> **Trade-off**: The coherence bound ({% term(url="#prop-56", def="OUTPOST Coherence Bound: 127-sensor mesh achieves fleet-wide health convergence within 3 gossip rounds when fewer than 42 nodes are Byzantine") %}Proposition 56{% end %}) shows divergence grows with both event rate and partition duration. Deploying more fusion nodes (increasing \\(k\\)) shrinks the sensor-to-fusion ratio and bounds divergence — but each fusion node adds hardware cost and maintenance burden.
