...
**Permanent isolation**: If a quarantined node's partition duration accumulator {% katex() %}T_{\mathrm{acc}}{% end %} exceeds {% katex() %}Q_{0.95}{% end %} (the Proposition 59 Weibull circuit-breaker threshold from Part 3) without re-sync completing, the node self-transitions to Terminal Safety State (Definition 36, Part 3): ceases all write operations, freezes its CRDT state, and broadcasts a `QUARANTINE_PERMANENT` flag to any reachable peers for audit logging. This prevents a permanently isolated node's stale state from contaminating fleet state on a surprise reconnect after an operationally long isolation.
...
3. **Corroboration currency**: every \\(i \in Q\\) has passed peer validation ({% katex() %}\kappa(c_i, \cdot) \geq k_{\mathrm{accept}}{% end %}) within the last {% katex() %}T_{\mathrm{stale}}{% end %} seconds (Prop 40)
...
- Causal window: \\(\tau_Q = 11.2\\)s (Contested {% katex() %}T_{\mathrm{stale}}{% end %} from Prop 40)
