...
Monitor observes via sensors and health metrics ([self-measurement infrastructure](@/blog/2026-01-22/index.md)). Analyze transforms raw metrics into diagnoses — "Battery 3.21V *(illustrative value)*" becomes "Drone 23 fails in 8 min *(illustrative value)*, probability 0.94 *(illustrative value)*." Plan generates options and selects the best expected outcome. Execute applies remediation, coordinates with affected components, and verifies success.
...
The proposition computes the maximum safe controller gain {% katex() %}K_{\text{ctrl}}{% end %} for a given feedback delay {% katex() %}\tau{% end %}, establishing the ceiling above which fault/heal flapping emerges from overcorrection. For {% katex() %}\tau = T_{\text{tick}}{% end %} the ceiling is {% katex() %}K_{\text{ctrl,max}} = 0.5{% end %} *(illustrative value)*; for {% katex() %}\tau = 2T_{\text{tick}}{% end %} it falls to {% katex() %}K_{\text{ctrl,max}} = 0.33{% end %} *(illustrative value)*. The formula gives the stability ceiling, not a recommended operating point.
...
*Watch out for*: the stability trade-off comes at the cost of healing speed — in Denied regime (\\(\tau \to \infty\\)), the gain ceiling approaches zero, meaning each {% term(url="#term-mape-k", def="Monitor-Analyze-Plan-Execute with Knowledge Base; the four-phase autonomic control loop enabling self-healing without central coordination") %}MAPE-K{% end %} tick corrects only a negligible fraction of the error state, and recovery may not complete within the healing deadline from {% term(url="#prop-21", def="Healing Deadline: healing must complete within the failure window minus a safety buffer; if no action fits the window, escalate to a faster intervention") %}Proposition 21{% end %} before the failure becomes irreversible.
...
The definition models round-trip delay as a heavy-tail Pareto distribution fitted from {% term(url="#term-mape-k", def="Monitor-Analyze-Plan-Execute with Knowledge Base; the four-phase autonomic control loop enabling self-healing without central coordination") %}MAPE-K{% end %} logs, enabling robust gain selection via {% term(url="#prop-23", def="Robust Gain Scheduling under Stochastic Delay: the scheduled gain derived from the stability margin maintains stability with high probability under Weibull-distributed delays") %}Proposition 23{% end %} and guarding against gain under-design when mean-delay assumptions underestimate P99 delay by \\(3{-}10\\times\\) when tail index {% katex() %}\alpha < 1.5{% end %}. The tail index {% katex() %}\alpha{% end %} ({% katex() %}\alpha < 2{% end %} means infinite variance) and the hardware-limited delay floor {% katex() %}\tau_{\min}{% end %} are both fitted from log data. Log-log plots of delay data distinguish distributions: a straight line confirms Pareto; curvature signals Weibull — and each requires a different gain formula.
...
*Watch out for*: the gain bound is derived for a single healing loop with a fixed delay \\(\tau\\); concurrent healing loops sharing one CPU raise the effective aggregate gain to {% katex() %}K_{\text{eff}} \approx K_{\text{ctrl}} \cdot N_{\text{concurrent}}{% end %}, which can exceed the stability ceiling even when each individual loop satisfies the per-loop bound — the multi-loop stability condition must be checked separately and is not a consequence of per-loop compliance alone.
...
*Using mean delay to set gain is unsafe in contested regimes — {% term(url="@/blog/2026-01-15/index.md#scenario-raven", def="47-drone surveillance swarm; loses backhaul mid-mission and must maintain coordinated operations without command authority") %}RAVEN{% end %}'s P99 delay can be \\(13\\times\\) *(illustrative value)* the mean, collapsing the safe gain ceiling to near zero.*

{% katex(block=true) %}
K_{\mathrm{robust}}(C, \delta_\text{inst}) \leq \frac{1}{1 + \tau_{1-\delta_\text{inst}}(C)/T_{\text{tick}}}
{% end %}

The proposition derives a conservative loop gain using the {% katex() %}P(1-\delta_\text{inst}){% end %} delay quantile instead of the mean, guaranteeing stability for {% katex() %}(1-\delta_\text{inst}){% end %} of all delay realizations and eliminating intermittent instability from rare long-tail events that a mean-based gain cannot handle. The instability tolerance {% katex() %}\delta_\text{inst} = 0.01{% end %} *(illustrative value)* targets 99% stability; {% katex() %}\tau_{1-\delta_\text{inst}}{% end %} follows from the Pareto or Weibull quantile formula. At {% katex() %}\delta_\text{inst} = 0.01{% end %} in a 5-second {% term(url="#term-mape-k", def="Monitor-Analyze-Plan-Execute with Knowledge Base; the four-phase autonomic control loop enabling self-healing without central coordination") %}MAPE-K{% end %} loop, the bound still permits approximately 3.6 instability events per day *(illustrative value)* — safety-critical deployments require {% katex() %}\delta_\text{inst} = 0.001{% end %} *(illustrative value)*.
...
For {% term(url="@/blog/2026-01-15/index.md#scenario-raven", def="47-drone surveillance swarm; loses backhaul mid-mission and must maintain coordinated operations without command authority") %}RAVEN{% end %} ({% katex() %}\tau_{\min} = 0.2{% end %}s *(illustrative value)*, \\(\alpha = 1.6\\) *(illustrative value)*, \\(\delta_\text{inst} = 0.01\\) *(illustrative value)*):
...
For {% term(url="@/blog/2026-01-15/index.md#scenario-raven", def="47-drone surveillance swarm; loses backhaul mid-mission and must maintain coordinated operations without command authority") %}RAVEN{% end %} (\\(N = 47\\) *(illustrative value)*, \\(\delta_{\text{fleet}} = 0.01\\) *(illustrative value)*, \\(\alpha = 1.6\\) *(illustrative value)*, \\(\tau_{\min} = 0.2\\) s *(illustrative value)*): {% katex() %}\delta_{\text{node}} \approx 2.13 \times 10^{-4}{% end %} *(theoretical bound)*, giving:
...
The corollary replaces {% katex() %}\tau_{1-\delta_\text{inst}}{% end %} with {% katex() %}\tau^+_{1-\delta_\text{inst}}{% end %} in Contested-regime gain computations, reverting to {% term(url="#prop-23", def="Robust Gain Scheduling under Stochastic Delay: the scheduled gain derived from the stability margin maintains stability with high probability under Weibull-distributed delays") %}Proposition 23{% end %}'s point estimate when {% katex() %}k \geq k_{\min}{% end %}. The minimum reliable tail-observation count is {% katex() %}k_{\min} = 15{% end %} *(theoretical bound)* (Hill SE below 26%); the confidence interval uses \\(\beta = 0.10\\) *(illustrative value)* (90% CI). For {% term(url="@/blog/2026-01-15/index.md#scenario-raven", def="47-drone surveillance swarm; loses backhaul mid-mission and must maintain coordinated operations without command authority") %}RAVEN{% end %} in the Contested window, \\(n_{\text{obs}} \approx 19\\) and \\(k \approx 4\\) *(illustrative value)* — below {% katex() %}k_{\min}{% end %} — placing the estimator below its reliable threshold; Severity suppression is accordingly the correct operating mode, not a conservative simplification. As {% katex() %}\mathrm{SE}(\hat{\alpha}){% end %} grows with sparse observations, the Execute dead-band from {% term(url="#def-38", def="MAPE-K Predictive Dead-Band: region around setpoint where the MAPE-K controller takes no action to prevent chattering under stochastic delay") %}Definition 38{% end %} widens in proportion, ensuring the Execute phase becomes more conservative under estimation uncertainty.

*Watch out for*: the confidence adjustment relies on the Hill estimator's Gaussian approximation {% katex() %}\mathrm{SE}(\hat{\alpha}) \approx \hat{\alpha}/\sqrt{k}{% end %}, which holds only when \\(k\\) is large enough for the Central Limit Theorem to apply; for {% term(url="@/blog/2026-01-15/index.md#scenario-raven", def="47-drone surveillance swarm; loses backhaul mid-mission and must maintain coordinated operations without command authority") %}RAVEN{% end %} in Contested regime with \\(k \approx 4\\) tail observations, this approximation is unreliable — the confidence-adjusted quantile \\(\tau^+\\) is not a statistically rigorous bound but a conservative heuristic, and the correct response when {% katex() %}k < k_{\min}{% end %} is Severity suppression, not reliance on the CI formula.
...
The threshold sets the maximum safe age for health reports beyond which acting on stale data risks triggering a wrong healing decision from ghost state of a fault that has already self-resolved. For {% term(url="@/blog/2026-01-15/index.md#scenario-raven", def="47-drone surveillance swarm; loses backhaul mid-mission and must maintain coordinated operations without command authority") %}RAVEN{% end %}, {% katex() %}T_{\text{heal}} = 30\text{ s}{% end %} and {% katex() %}k = 3{% end %} *(illustrative value)* yield a maximum safe data age of {% katex() %}T_{\text{stale}} = 10\text{ s}{% end %} *(illustrative value)*. Stale detection is structurally inapplicable without monotonic timestamps on every health message — the threshold formula presupposes time-ordered records.
...
The dCBF condition {% katex() %}h_q(A_q x + B_q u) \geq (1-\gamma_{\text{cbf}})h_q(x){% end %} is evaluated before every Execute phase; if violated, \\(K_{\text{ctrl}}\\) is reduced via the CBF-QP closed form until the condition holds. The contraction rate {% katex() %}\gamma_{\text{cbf}} \in (0,1){% end %} governs the tightness of the constraint on admissible \\(K_{\text{ctrl}}\\) — smaller \\(\gamma_{\text{cbf}}\\) means tighter contraction; {% katex() %}\gamma_{\text{cbf}} = 0.05{% end %} *(illustrative value)* is a safe default for 5 s {% term(url="#term-mape-k", def="Monitor-Analyze-Plan-Execute with Knowledge Base; the four-phase autonomic control loop enabling self-healing without central coordination") %}MAPE-K{% end %} ticks, with runtime cost bounded by one {% katex() %}6\times6{% end %} quadratic form (36 multiplications, \\(<20\\,\mu\\)s on Cortex-M4 at L1 throttle *(illustrative value)*). The evaluation cost equals that of computing \\(\rho_q(t)\\) — the safety filter adds negligible overhead alongside the stability margin computation already performed at each tick.
...
The scheduler looks up {% katex() %}K_{\mathrm{gs}}{% end %} from a precomputed 100-entry \\(\rho_q\\)-indexed table at each {% term(url="#term-mape-k", def="Monitor-Analyze-Plan-Execute with Knowledge Base; the four-phase autonomic control loop enabling self-healing without central coordination") %}MAPE-K{% end %} tick, replacing the static \\(K_{\text{ctrl}}\\) in the healing actuator; the table fits in 400 bytes *(illustrative value)* of MCU flash with no runtime LMI required. Full gain (\\(\Phi = 1\\)) applies when \\(\rho > 0.5\\) — no derate in the safe interior; the gain derates linearly as \\(\rho\\) falls below 0.5; healing is suspended (zero gain) when \\(\rho < 0\\). A DEFER rate above 5% per hour *(illustrative value)* (\\(\rho_q < 0\\) triggering suspension) indicates either {% katex() %}K_{\text{nominal}}{% end %} is too aggressive for the current mode or the \\(P_q\\) matrices are miscalibrated from stale field data.
...
**Logic:** The dCBF condition {% katex() %}h_q(A_q x + B_q u) \geq (1 - \gamma_{\text{cbf}}) h_q(x){% end %} enforces a per-tick safety margin. The gain scheduler {% katex() %}K_{\mathrm{gs}} = \eta \cdot K_{\max}^{\mathrm{LTI}} \cdot \Phi(\rho_q){% end %} then linearly scales healing authority down as \\(\rho_q\\) approaches zero.
...
The proposition formally certifies that no healing action fires while the system is outside its Stability Region in any capability mode — an invariant required as evidence for Level 3+ {% term(url="@/blog/2026-02-19/index.md#def-104", def="Field Autonomic Certification: process verifying an edge node's autonomic stack meets minimum capability requirements before granting operational authority") %}Field Autonomic Certification{% end %}. The precondition {% katex() %}x(0) \in \mathcal{R}_{q(0)}{% end %} is verified at boot (Phase 0 of FAC); {% katex() %}d_{\max} \leq 5{% end %} *(illustrative value)* ticks of DEFER guarantees re-entry for all {% term(url="@/blog/2026-01-15/index.md#scenario-raven", def="47-drone surveillance swarm; loses backhaul mid-mission and must maintain coordinated operations without command authority") %}RAVEN{% end %}/{% term(url="@/blog/2026-01-15/index.md#scenario-convoy", def="12-vehicle autonomous ground convoy in contested mountainous terrain; active electronic warfare requires autonomous operation at every command level") %}CONVOY{% end %}/{% term(url="@/blog/2026-01-15/index.md#scenario-outpost", def="127-sensor perimeter mesh at a forward base; sustains autonomous threat detection under sustained jamming and denied external communications") %}OUTPOST{% end %} configurations. A stability margin {% katex() %}\rho_q(t){% end %} trending from 0.85 to 0.40 *(illustrative value)* over 90 minutes under sustained L1 throttle is the early-warning signal that pure LTI analysis cannot surface: {% katex() %}K = 0.30 < K_{\max} = 0.33{% end %} *(illustrative value)* appears healthy at every tick until the loop suddenly destabilizes.
...
For RAVEN, rotor-failure propagation takes approximately 200 ms *(illustrative value)* while {% katex() %}T_{\text{tick}} = 5\,\mathrm{s}{% end %} *(illustrative value)*, so this condition is violated by \\(25\\times\\) *(illustrative value)*. The certificate guarantees the swarm was safe at the last check interval, not that it remains safe until the next.
...
*Watch out for*: the invariant holds only when the precondition {% katex() %}x(0) \in \mathcal{R}_{q(0)}{% end %} is verified at boot and the dCBF check runs without interruption at every tick; for {% term(url="@/blog/2026-01-15/index.md#scenario-raven", def="47-drone surveillance swarm; loses backhaul mid-mission and must maintain coordinated operations without command authority") %}RAVEN{% end %}, rotor failure propagates in ~200 ms while {% katex() %}T_{\text{tick}} = 5\,\text{s}{% end %} — a rotor fault that occurs mid-tick renders the safety certificate vacuously true at the last check but physically violated at the next, meaning the certificate bounds the state at check times, not between them.
...
> **Physical translation**: {% katex() %}\text{MTTU gain} = T_{\text{human detect}} / T_0{% end %} — the watchdog replaces human detection time with a hardware timer period. For RAVEN, a human operator might take 5 minutes *(illustrative value)* to notice a hung loop; the hardware WDT fires in 100 ms *(illustrative value)*. The gain is {% katex() %}300\,\text{s} / 0.1\,\text{s} = 3000\times{% end %} *(illustrative value)*. Smaller \\(T_0\\) means faster detection but tighter timing requirements on the MAPE-K loop's heartbeat — the loop must reliably complete and write its heartbeat within every \\(T_0\\) window even under peak load.
...
*Watch out for*: the MTTU bound {% katex() %}\mathbb{E}[T_{\text{unprotected}}] \leq T_0{% end %} holds only when the {% term(url="#term-mape-k", def="Monitor-Analyze-Plan-Execute with Knowledge Base; the four-phase autonomic control loop enabling self-healing without central coordination") %}MAPE-K{% end %} loop's heartbeat write runs at the highest scheduler priority; if the heartbeat can be preempted by a resource-intensive healing action, the effective exposure window extends beyond \\(T_0\\) — precisely during high-load conditions such as simultaneous healing loops and cascade failures, when fast watchdog response is most critical.
...
The condition requires \\(\varepsilon = 0.01\\) *(illustrative value)*, {% katex() %}\eta_{\min} = 0.001 \cdot V(S){% end %} *(illustrative value)* (0.1% fleet stress reduction per action), and {% katex() %}\Delta r_{\max} = 0.2{% end %} *(illustrative value)* (maximum single-transfer fraction). \\(V(S)\\) is computed from the {% term(url="@/blog/2026-01-22/index.md#def-24", def="Gossip Health Protocol: epidemic dissemination where each node contacts random neighbors to propagate state; convergence guaranteed in logarithmic rounds") %}gossip{% end %} health vector; peer data is bounded-stale by {% katex() %}\tau_\text{stale}^\text{max}{% end %} ({% term(url="@/blog/2026-01-22/index.md#prop-14", def="Maximum useful staleness bound: gossip data older than the staleness threshold degrades anomaly detection below acceptable sensitivity") %}Proposition 14{% end %}); the HAC check is \\(O(N)\\) in gossip vector size, constant time for a fixed fleet. A non-decreasing \\(V\\) trace is the diagnostic signature of either a HAC implementation bug or a fault not addressable by load redistribution — both warrant escalation to severity S3 ({% term(url="#def-44", def="Healing Action Severity: ordinal scale classifying healing actions by resource cost and mission disruption, used to select minimum-impact interventions first") %}Definition 44{% end %}).
...
*Watch out for*: the finite convergence bound {% katex() %}\lceil V(S(0))/\eta_{\min} \rceil{% end %} steps assumes the load state \\((\ell_i, d_i, q_i)\\) remains constant during each healing step interval; under bursty traffic or simultaneous external faults, \\(V(S)\\) may increase between admitted steps even when HAC was satisfied at firing time — the certificate guarantees forward progress on average, not monotone decrease under all traffic patterns, and the convergence step count can exceed the bound when external perturbations consistently reverse a fraction of each healing step's improvement.
...
*Watch out for*: the \\(\mathcal{L}_0\\)-tier deadline guarantee {% katex() %}P(\text{miss deadline}) = 0{% end %} holds only when at most one action occupies the {% katex() %}P_{ij} = 1.0{% end %} priority level at any given time; when multiple components simultaneously declare \\(\mathcal{L}_0\\)-tier emergencies (simultaneous thermal and structural faults), they compete as equal-priority actions and the absolute deadline guarantee degrades to the probabilistic bound — the zero-miss guarantee is absolute only for a single top-tier action, not for a concurrent set of them.
...
The proposition sets the minimum dead-band window after each healing action from the round-trip feedback delay {% katex() %}\tau_{\text{fb}}{% end %}, establishing the floor below which a second action fires before the first effect is observable. For {% term(url="@/blog/2026-01-15/index.md#scenario-raven", def="47-drone surveillance swarm; loses backhaul mid-mission and must maintain coordinated operations without command authority") %}RAVEN{% end %} ({% katex() %}\tau_{\text{fb}} = 5\text{ s}{% end %} *(illustrative value)*), the minimum refractory period is {% katex() %}\tau_{\text{ref,min}} = 10\text{ s}{% end %} *(illustrative value)*. The \\(2\\times\\) floor is derived for linear delay chains; healing actions with nonlinear side effects such as gossip storms may require {% katex() %}3\tau_{\text{fb}}{% end %} *(illustrative value)* — the \\(2\\times\\) bound is the theoretical minimum, not a general operating recommendation.
...
The accumulator counts pending healing requests above the dead-band threshold, capped at {% katex() %}Q_{\text{aw}}{% end %}, and dispatches only when {% katex() %}Q_d > 0{% end %} and the refractory timer has expired — preventing burst discharge from releasing a suppressed queue all at once. The anti-windup cap {% katex() %}Q_{\text{aw}}{% end %} is set between 5 and 10 actions *(illustrative value)*, with the constraint {% katex() %}Q_{\text{aw}} \cdot T_{\text{exec}} < T_{\text{heal}}{% end %} bounding total discharge time within the healing deadline. {% katex() %}Q_d > Q_{\text{aw}}/2{% end %} sustained for 3+ ticks *(illustrative value)* is the diagnostic signature of a persistent fault that load redistribution alone cannot resolve, warranting escalation to a higher severity level.
...
The proposition replaces the fixed {% katex() %}2\tau_{\mathrm{fb}}{% end %} floor with a state-dependent lower bound that ensures the stability margin recovers above {% katex() %}\rho_{\min}{% end %} before the next healing action; larger healing actions that consume more stability margin automatically produce longer refractory periods. The minimum safe margin is {% katex() %}\rho_{\min} = 0.2{% end %} *(illustrative value)*; the singularity floor is {% katex() %}\rho_\varepsilon = 10^{-3}{% end %} *(illustrative value)*; \\(\gamma_{\text{cbf}}\\) follows from {% term(url="#def-39", def="Discrete Control Barrier Function: safety function enforcing mode-invariant stability across capability level transitions in the switched system") %}Definition 39{% end %}. For {% term(url="@/blog/2026-01-15/index.md#scenario-raven", def="47-drone surveillance swarm; loses backhaul mid-mission and must maintain coordinated operations without command authority") %}RAVEN{% end %} L3 with \\(\gamma_{\text{cbf}} = 0.05\\) *(illustrative value)* and a large action dropping \\(\rho\\) to 0.1 *(illustrative value)*, the CBF-derived floor is {% katex() %}\tau_{\mathrm{ref}}^{\mathrm{CBF}} \approx 70\,\text{s}{% end %} *(illustrative value)* versus the {% term(url="#prop-30", def="Anti-Windup Oscillation Bound: healing dead-band with adaptive refractory backoff prevents integrator windup; refractory floor is 2τ_fb") %}Proposition 30{% end %} floor of 10 s *(illustrative value)*. The gap between {% katex() %}\tau_{\mathrm{ref}}^{\mathrm{CBF}}{% end %} and {% katex() %}2\tau_{\mathrm{fb}}{% end %} quantifies how much stability margin the action consumed and is the primary diagnostic for oversized healing gains.
...
**Hyper-aggressive failure** (under-refractory): physical damage slows \\(\rho_q\\) recovery below the nominal rate. Example — RAVEN drone motor at 60% *(illustrative value)* thrust efficiency shifts the dominant eigenvalue from \\(|\lambda| = 0.95\\) *(illustrative value)* to \\(|\lambda| = 0.98\\) *(illustrative value)*; the ticks required for \\(\rho: 0.10 \to 0.20\\) extend from {% katex() %}\lceil \ln 2 / (-\ln(1-\gamma_{\text{cbf}})) \rceil = \lceil \ln 2 / 0.051 \rceil = 14{% end %} *(theoretical bound)* ticks (70 s) to {% katex() %}\lceil \ln 2 / 0.020 \rceil = 35{% end %} *(theoretical bound)* ticks (175 s). The formula fires the next healing action at tick 14 when true \\(\rho \approx 0.15\\) *(illustrative value)* — still below \\(\rho_{\min} = 0.20\\). A second actuation on an under-margined plant can collapse the voltage rail.
...
The definition doubles the refractory window after each consecutive recovery failure, capped at {% katex() %}\tau_{\text{ref}}^{\max}{% end %}, preventing rapid healing storms that exhaust the action budget on a persistent fault. The initial refractory period is {% katex() %}\tau_{\text{ref}}(0) = 2\tau_{\text{fb}}{% end %} ({% term(url="#prop-30", def="Anti-Windup Oscillation Bound: healing dead-band with adaptive refractory backoff prevents integrator windup; oscillation amplitude decays with each refractory cycle") %}Proposition 30{% end %} floor); doubling factor {% katex() %}\beta = 2{% end %} *(illustrative value)*; the ceiling is {% katex() %}\tau_{\text{ref}}^{\max} = 10\tau_{\text{ref}}(0){% end %} *(illustrative value)*; the counter resets on genuine recovery. A backoff counter exceeding 3 *(illustrative value)* is the diagnostic threshold for human escalation — it indicates a fault that the autonomic loop cannot resolve autonomously.
...
The sliding-window derivative estimates whether the confidence score is rising ({% katex() %}\dot{\theta} > 0{% end %}) or falling ({% katex() %}\dot{\theta} < 0{% end %}), distinguishing worsening faults from self-recovering transients in the Analyze phase before the actuation hold condition is checked. The window {% katex() %}w = 5{% end %} samples *(illustrative value)* is the empirically calibrated optimum for the series scenarios: {% katex() %}w = 3{% end %} is too noisy under vibration, and {% katex() %}w = 10{% end %} is too slow to track fast-moving faults — larger \\(w\\) reduces noise but increases response lag in a direct trade-off.
...
The actuation hold condition suppresses Execute when {% katex() %}\dot{\theta}{% end %} falls faster than {% katex() %}\gamma_{\text{damp}}{% end %}, meaning confidence is recovering fast enough to self-resolve before {% katex() %}\tau_{\text{confirm}}{% end %} elapses, eliminating false actuation on transient spikes that briefly cross {% katex() %}\theta_H{% end %} but are already recovering. For {% term(url="@/blog/2026-01-15/index.md#scenario-convoy", def="12-vehicle autonomous ground convoy in contested mountainous terrain; active electronic warfare requires autonomous operation at every command level") %}CONVOY{% end %}, {% katex() %}\Delta\theta=0.20{% end %}, \\(w=5\\), {% katex() %}T_{\text{tick}}=1\text{ s}{% end %} give {% katex() %}\gamma_{\text{damp}}=0.02\text{ s}^{-1}{% end %} *(illustrative value)*; in {% term(url="@/blog/2026-01-15/index.md#scenario-convoy", def="12-vehicle autonomous ground convoy in contested mountainous terrain; active electronic warfare requires autonomous operation at every command level") %}CONVOY{% end %} testing this hold suppressed 67% of reroute commands *(illustrative value)* that would have been false positives under pure threshold triggering.
...
*Watch out for*: the liveness guarantee requires {% katex() %}\tau_{\text{ref}}^{\max} \geq T_{\text{mission}}/2{% end %}; when the ceiling cap {% katex() %}\tau_{\text{ref}}^{\max} = 10 \times \tau_{\text{ref}}(0){% end %} is reached before the backoff window spans the remaining mission time, the system can exhaust \\(n_{\max}\\) retries without the refractory period naturally terminating the loop — in this case, transition to the Terminal Safety State is triggered by the {% term(url="#def-50", def="Minimum Viable System: smallest component subset whose combined capability meets the survival threshold, defining the healing algorithm's priority boundary") %}MVS{% end %} floor rather than the backoff bound, and the formal liveness certificate does not apply.
...
*Watch out for*: the \\(O(\ln |V|)\\) approximation guarantee requires the capability function to be submodular; when components have joint prerequisites (neither alone contributes to a capability but together they unlock it), submodularity fails and the greedy algorithm may return a set \\(2\text{–}3\times\\) larger than the true {% term(url="#def-50", def="Smallest set of components that must remain operational to sustain the mission-critical survival capability; defines the healing algorithm priority boundary") %}MVS{% end %} — such pairs must be pre-identified and treated as compound components before running greedy, and failure to verify submodularity produces a silently suboptimal MVS that wastes healing resources on unnecessary inclusions.
...
The definition gives total autonomic overhead in milliwatts at each capability level {% katex() %}k{% end %} from L0 to L4, with the constraint {% katex() %}P_k \leq R_{\text{total}} - R_{\text{mission}}{% end %} bounding which tiers are feasible given available power. For {% term(url="@/blog/2026-01-15/index.md#scenario-raven", def="47-drone surveillance swarm; loses backhaul mid-mission and must maintain coordinated operations without command authority") %}RAVEN{% end %}: L0 {% katex() %}\approx 0.1{% end %} mW *(illustrative value)*; L1 {% katex() %}\approx 2{% end %} mW *(illustrative value)*; L2 {% katex() %}\approx 8{% end %} mW *(illustrative value)*; L3 {% katex() %}\approx 20{% end %} mW *(illustrative value)*; L4 {% katex() %}\approx 42{% end %} mW *(illustrative value)*. Simulation systematically underestimates {% katex() %}P_k{% end %} by \\(2{-}3\\times\\) *(illustrative value)* due to radio idle drain that does not appear in logic-level models — empirical measurement on real hardware is required for accurate tier feasibility assessment.
...
*Watch out for*: the survival time formula uses autonomic overhead power values {% katex() %}\mathcal{P}_k{% end %} that simulation systematically underestimates by \\(2\text{–}3\times\\) due to radio idle drain not captured in logic-level models — a node that computes a 32-hour survival window from simulated {% katex() %}\mathcal{P}_0 = 0.1\,\text{mW}{% end %} may in practice have only 10–15 hours because the hardware radio idle current alone exceeds the simulated baseline; empirical {% katex() %}\mathcal{P}_k{% end %} measurements on target hardware are required before the throttle thresholds are used operationally.
...
*Watch out for*: the \\(9.3\times\\) multiplier assumes {% katex() %}\mathcal{P}_0 \approx 0.1\,\text{mW}{% end %} for survival mode, but this floor is also a simulation estimate — if the radio's idle-receive drain is mischaracterized at {% katex() %}\mathcal{P}_0{% end %} just as it is at {% katex() %}\mathcal{P}_4{% end %}, both ends of the ratio are wrong and the true multiplier may be substantially lower than \\(9.3\times\\); measure {% katex() %}\mathcal{P}_0{% end %} and {% katex() %}\mathcal{P}_4{% end %} independently on target hardware before concluding that a single throttle step provides near-order-of-magnitude survival extension.
...
The proposition reduces {% term(url="#term-mape-k", def="Monitor-Analyze-Plan-Execute with Knowledge Base; the four-phase autonomic control loop enabling self-healing without central coordination") %}MAPE-K{% end %} monitoring frequency proportionally to the remaining resource fraction {% katex() %}R(t){% end %}, preventing the autonomic loop from consuming more power than the primary mission at low battery levels. The minimum frequency {% katex() %}f_{\min}{% end %} applies only during active critical failure; the scaling function {% katex() %}\alpha(R){% end %} is piecewise linear between {% katex() %}R_{\text{floor}}{% end %} and {% katex() %}R_{\text{crit}}{% end %}. A frequency drop to {% katex() %}f_{\min}{% end %} visible in telemetry is the earliest leading indicator of an energy-budget crisis — it is observable before the resource constraint becomes binding.
...
The proposition bounds total cascade duration as the sum of wait, convergence, and actuation times across {% katex() %}K^*{% end %} stages, with {% katex() %}D_{\text{recovery}} = T_{\text{heal}} - T_{\text{margin}}{% end %} ({% term(url="#prop-21", def="Healing Deadline: upper bound on time-to-resolution for healing actions of given severity, derived from MAPE-K tick rate and action queue depth") %}Proposition 21{% end %}) and {% katex() %}K^*{% end %} equal to the number of cascaded recovery stages in the dependency graph. Each stage requires a hard per-stage timeout — a stage without one that blocks indefinitely on a failed upstream dependency is the single most common cascade failure mode in practice.
