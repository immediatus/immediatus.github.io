...
*Active constraint evaluation: given system state {% katex() %}(\mathcal{C}, \mathcal{R}, \mathcal{A}){% end %} (connectivity, resources, autonomy level), a constraint {% katex() %}c \in \mathbb{C}{% end %} is **active-binding** if (1) its gate validator \\(V_c(S) < \\theta_c\\) (the gate is not currently passing) and (2) the system's current capability level \\(L\\) satisfies \\(\\sigma(c) \\leq L\\) (the constraint is in-scope for the current phase). The active constraint set is {% katex() %}\mathbb{C}_\text{act}(t) = \{c \in \mathbb{C} : V_c(S) < \theta_c \text{ and } \sigma(c) \leq L(t)\}{% end %}.*
