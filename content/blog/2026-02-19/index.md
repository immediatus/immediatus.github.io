...
>
> **Connection to safe state**: A system that cannot achieve Phase 0 (\\(G_0\\) permanently fails) enters the Terminal Safety State ([Definition 36](@/blog/2026-01-29/index.md#def-36)) and awaits physical inspection. There is no defined operational mode below Phase 0.
...
|:---|:---|:---|:---|
| L0 | Disabled — physical interlock only | Active | Never — hardware override in effect (Def 54, Prop 62) |
...
**Bootstrap approach:** Derive initial approximations {% katex() %}\hat{S}^{(0)}{% end %}, {% katex() %}\hat{C}^{(0)}{% end %} from simulation or design specifications. Develop self-measurement assuming {% katex() %}\hat{C}^{(0)}{% end %}, validate communication assuming {% katex() %}\hat{S}^{(0)}{% end %}, update estimates, and iterate until successive approximations change less than a predefined tolerance (e.g., 1% of threshold value). This converges in practice because self-measurement quality and communication quality are weakly coupled at initialization — neither depends strongly on the other until the system is near operational.
...
**Resource floor.** The composite resource availability \\(R(t)\\) (Definition 19b) has a critical threshold {% katex() %}R_{\text{crit}} \approx 0.2{% end %} at which survival-mode shedding activates. The operationally important threshold is earlier: Proposition 21 establishes the autonomic overhead ceiling {% katex() %}R_{\text{autonomic}}^{\max} = R_{\text{total}} - R_{\text{mission}}^{\min}{% end %}. Below {% katex() %}R(t) = 1.75\,R_{\text{crit}} \approx 0.35{% end %}, the healing planner's action feasibility constraint {% katex() %}g_2(a):\, \text{Cost}(a) \leq R_{\text{available}}{% end %} begins rejecting Severity 2 and above actions (Definition 9). The system is above {% katex() %}R_{\text{crit}}{% end %} but already healing-impaired.
...
| :--- | :--- | :--- |
| Stochastic partition duration | Weibull Semi-Markov model (Definition 66) + Proposition 59 (Weibull Circuit Breaker) | [P1](@/blog/2026-01-15/index.md), [P3](@/blog/2026-01-29/index.md) |
...
| Multi-failure cascade dead-ends | Terminal Safety State (Definition 36) + Hardware Veto (Proposition 62) | [P3](@/blog/2026-01-29/index.md) |
...
**SCALEFAST scenario**: A cloud-to-edge migration project using this framework applies the constraint sequence in reverse: capabilities built for the cloud-native environment must be re-validated against the edge prerequisite graph. The Weibull circuit breaker (Proposition 59) fires during SCALEFAST migration testing when the new edge nodes encounter partition durations that the cloud-native codebase was never designed for — the FAC checklist (Definition 37, items C8–C10) identifies these gaps before production deployment.
