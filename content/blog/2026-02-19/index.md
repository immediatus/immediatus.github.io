...
\\(\\rho_q(t)\\) is the discrete-time CBF safety margin sampled at each MAPE-K tick. It is the discretized equivalent of Proposition 9's continuous Lyapunov margin ([Self-Healing Without Connectivity](@/blog/2026-01-29/index.md#prop-9)). Reducing \\(\\rho_\\text{min}\\) below 0.2 is not recommended without re-analysis against Proposition 9's stability conditions — the discrete sampling interval \\(T_\\text{tick}\\) means the true continuous margin may be lower than the sampled value.

> **Calibration for other platforms.** The values \\(R_\\text{min} = 0.2\\), \\(C_\\text{conn} = 0.3\\), \\(\\rho_\\text{min} = 0.2\\) are RAVEN-specific. For other deployments: (1) set \\(\\rho_\\text{min}\\) from Proposition 9's stability condition for your capability level; (2) set \\(R_\\text{min}\\) as the resource level below which healing actions cannot execute (Definition 115 load-shedding threshold); (3) set \\(C_\\text{conn}\\) as the minimum link capacity needed for one gossip packet per MAPE-K tick. Example guidance — CONVOY (12 vehicles): \\(\\rho_\\text{min} = 0.15\\) (larger vehicles, slower failure modes), \\(R_\\text{min} = 0.25\\) (fuel-constrained), \\(C_\\text{conn} = 0.4\\) Mbit/s (mesh radio).
...
> **Deriving \\(\Psi_\text{fail}\\).** \\(\Psi_\text{fail}\\) is the confidence threshold below which the system can no longer guarantee safe-state reachability (Definition 124, [Self-Healing Without Connectivity](@/blog/2026-01-29/index.md#def-124)). Derivation procedure: (1) Identify the minimum Lyapunov stability margin \\(\rho_\text{min}\\) required by Proposition 9; (2) Identify the minimum resource state \\(R_\text{min}\\) below which healing actions cannot be executed; (3) Set \\(\Psi_\text{fail}\\) as the \\(\Psi(t)\\) value that results when all three factors (R, C, \\(\rho_q\\)) are simultaneously at their respective minimums: {% katex() %}\Psi_\text{fail} = \min(1, 1 \cdot 1 \cdot 1) = 1{% end %} (trivially); therefore in practice, \\(\Psi_\text{fail}\\) is calibrated from mission reversibility constraints — set it at the \\(\Psi\\) value below which a bad autonomous decision cannot be corrected before mission end. For RAVEN: \\(\Psi_\text{fail} = 0.3\\) (calibrated from abort-window analysis).
...
> **Connection to HLC.** The Merkle root \\(M_\\text{op}\\) represents the cryptographic digest of the operation log consistent with the HLC timestamp (Definition 40, [Fleet Coherence Under Partition](@/blog/2026-02-05/index.md#def-40)). Commands whose Merkle roots reflect stale fleet state are rejected because their causal assumptions — what the fleet's state was at the moment of issuance — no longer hold. This extends the vector clock causality guarantee (Definition 13) to the command-authority domain. The Merkle root \\(M_\\text{edge}\\) includes a timestamp sourced from the HLC (Definition 40, [Fleet Coherence Under Partition](@/blog/2026-02-05/index.md#def-40)). When comparing \\(M_\\text{op}\\) to \\(M_\\text{edge}(t - \\Delta_\\text{prop})\\), also verify that the HLC timestamp embedded in \\(M_\\text{op}\\) falls within \\(T_\\text{trust}\\) of the current HLC value (Definition 98); reject commands whose Merkle root timestamp lies outside the trust window, as clock drift may have invalidated the causal assumption.
