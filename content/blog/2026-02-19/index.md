...
*Watch out for*: the invariant \\(\bigwedge_{j=0}^i G_j(S) = 1\\) is evaluated against the system's current state \\(S\\), not against the historical state at which each gate originally passed; a gate predicate that passed during controlled testing but depends on an environmental condition (ambient RF interference baseline, operator response latency, hardware temperature) may fail in the field while the development record shows it as green — the invariant certifies that the predicate passes now, not that it passed under the same conditions that will hold during deployment, and a predicate failure during production operation may reflect environment drift rather than capability regression.
...
*Watch out for*: the 15-second bound assumes a trained operator under moderate cognitive load; in high-stress scenarios (contested partition with simultaneous active threats), per-item processing time can rise from 2.1 s to 4–6 s, extending the bound to 28–42 s for \\(N_{\max} = 7\\) items — and the Shadow Mode pre-load that enables the 15-second target is only effective if the operator is attending to the autonomous system during the observation window rather than managing concurrent out-of-band communications.
